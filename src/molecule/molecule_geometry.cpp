...
double covalent_radius(const QString& element) {
  static const QHash<QString, double> radii{
      {QStringLiteral("H"), 0.31},  {QStringLiteral("B"), 0.84},  {QStringLiteral("C"), 0.76},
      {QStringLiteral("N"), 0.71},  {QStringLiteral("O"), 0.66},  {QStringLiteral("F"), 0.57},
      {QStringLiteral("Si"), 1.11}, {QStringLiteral("P"), 1.07},  {QStringLiteral("S"), 1.05},
      {QStringLiteral("Cl"), 1.02}, {QStringLiteral("Fe"), 1.32}, {QStringLiteral("Cu"), 1.32},
      {QStringLiteral("Zn"), 1.22}, {QStringLiteral("Br"), 1.20}, {QStringLiteral("I"), 1.39},
  };
...
bool MoleculeGeometry::is_valid() const { return !atoms.isEmpty(); }

bool MoleculeGeometry::has_vibration() const {
  return std::any_of(atoms.cbegin(), atoms.cend(), [](const MoleculeAtom& atom) {
    return atom.vibration.lengthSquared() > 1.0e-10f;
  });
}

QVector<QVector3D> MoleculeGeometry::positions_at_phase(float phase_radians,
                                                        float amplitude) const {
  const float displacement_scale = std::sin(phase_radians) * std::max(amplitude, 0.0f);
  QVector<QVector3D> positions;
  positions.reserve(atoms.size());
  for (const MoleculeAtom& atom : atoms) {
    positions.push_back(atom.position + atom.vibration * displacement_scale);
  }
  return positions;
}

bool load_xyz_molecule(const QString& path, MoleculeGeometry* geometry, QString* error_message) {
  if (!geometry) {
  ...
  *geometry = {};
  ...
  for (int atom_index = 0; atom_index < atom_count; ++atom_index) {
    if (stream.atEnd()) {
      set_error(
          error_message,
          QStringLiteral("XYZ ended before atom %1 of %2").arg(atom_index + 1).arg(atom_count));
      return false;
    }
    ...
    if (element.isEmpty() || !x_ok || !y_ok || !z_ok || !std::isfinite(x) || !std::isfinite(y) ||
        !std::isfinite(z) || std::abs(x) > 1.0e6 || std::abs(y) > 1.0e6 || std::abs(z) > 1.0e6) {
      set_error(error_message,
                QStringLiteral("Invalid XYZ atom record on line %1").arg(atom_index + 3));
      return false;
    }
    ...
    if (fields.size() >= 7) {
      ...
      bool dy_ok = false;
      bool dz_ok = false;
      const double dx = fields.at(4).toDouble(&dx_ok);
      const double dy = fields.at(5).toDouble(&dy_ok);
      const double dz = fields.at(6).toDouble(&dz_ok);
      if (!dx_ok || !dy_ok || !dz_ok || !std::isfinite(dx) || !std::isfinite(dy) ||
          !std::isfinite(dz) || std::abs(dx) > 1.0e6 || std::abs(dy) > 1.0e6 ||
          std::abs(dz) > 1.0e6) {
        set_error(error_message,
                  QStringLiteral("Invalid XYZ vibration vector on line %1").arg(atom_index + 3));
        return false;
      }
      vibration = QVector3D(float(dx), float(dy), float(dz));
    }

    parsed.atoms.push_back(
        MoleculeAtom{element, QVector3D(float(x), float(y), float(z)), vibration});
  }
