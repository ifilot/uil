...
float distance(float bounding_radius, float viewport_aspect, float distance_factor) {
    ...
    return std::max(
        fit_distance(radius, viewport_aspect) * factor,
        radius * kMinimumCameraClearance);
}

float zoomed_distance_factor(float current_factor, int angle_delta_y, int pixel_delta_y) {
    const float delta_steps = angle_delta_y != 0
        ? float(angle_delta_y) / 120.0f
        : float(pixel_delta_y) / 80.0f;
    return std::clamp(
        current_factor * std::pow(kWheelZoomPerStep, delta_steps),
        kMinimumDistanceFactor,
        kMaximumDistanceFactor);
}
